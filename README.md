...
```mermaid
graph TD
    A[Веб-приложение / API] --> B{Координатор <br/> (Shard Router/Proxy)};

    B -- Запрос с user_id --> C[Вертикальная группа:<br/>ПОЛЬЗОВАТЕЛИ];
    B -- Запрос с book_id --> D[Вертикальная группа:<br/>КНИГИ];
    B -- Запрос по магазинам --> E[Вертикальная группа:<br/>МАГАЗИНЫ И ЗАКАЗЫ];

    C --> F[Шард 1<br/>(user_id 1-1M)<br/>Сервер БД-1.1];
    C --> G[Шард 2<br/>(user_id 1M-2M)<br/>Сервер БД-1.2];
    C --> H[...];

    D --> I[Шард A<br/>(book_id 1-500k)<br/>Сервер БД-2.1];
    D --> J[Шард B<br/>(book_id 500k-1M)<br/>Сервер БД-2.2];
    D --> K[...];

    E --> L[Сервер БД-3];
    
    subgraph "Структура каждого шарда/сервера"
        F --> M1[Master: Запись];
        F --> N1[Slave 1: Чтение];
        F --> O1[Slave 2: Чтение/Резерв];
    endРежимы работы серверов базы данных
