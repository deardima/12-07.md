...
```mermaid
graph TD
    A["Веб-приложение / API"] --> B["Координатор (Shard Router/Proxy)"];
    
    B --> C["Группа: ПОЛЬЗОВАТЕЛИ"];
    B --> D["Группа: КНИГИ"];
    B --> E["Группа: МАГАЗИНЫ и ЗАКАЗЫ"];
    
    C --> F["Шард 1 (user_id 1-10) Сервер БД-1.1"];
    C --> G["Шард 2 (user_id 11-20) Сервер БД-1.2"];
    C --> H["..."];
    
    D --> I["Шард A (book_id 1-500) Сервер БД-2.1"];
    D --> J["Шард B (book_id 501-1000) Сервер БД-2.2"];
    D --> K["..."];
    
    E --> L["Сервер БД-3"];
    
    subgraph "Структура каждого шарда"
        F --> M["Master: Запись"];
        F --> N["Slave 1: Чтение"];
        F --> O["Slave 2: Резерв"];
    end
